...
class CommentControllerTest extends TestCase
{
    ...
    public function test_mentioning_a_real_team_member_by_name_creates_a_mention_row(): void
    {
        ...
        $response = $this->actingAs($owner)->postJson("/api/projects/{$project->id}/comments", [
    ...
    public function test_a_viewer_can_read_the_thread_but_not_post(): void
    {
        ...
        $this->actingAs($owner)->postJson("/api/projects/{$project->id}/comments", [
            'commentable_type' => 'project',
            'commentable_id' => $project->id,
            'body' => 'First comment',
        ])->assertCreated();
        ...
        $this->actingAs($viewer)
            ->getJson("/api/projects/{$project->id}/comments?commentable_type=project&commentable_id={$project->id}")
            ->assertOk()
            ->assertJsonCount(1);

        $this->actingAs($viewer)->postJson("/api/projects/{$project->id}/comments", [
            'commentable_type' => 'project',
            'commentable_id' => $project->id,
    ...
    public function test_a_comment_on_a_feature_request_from_a_different_project_is_rejected(): void
    {
        ...
        $this->actingAs($owner)->postJson("/api/projects/{$projectA->id}/comments", [
