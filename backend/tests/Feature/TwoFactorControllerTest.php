...
class TwoFactorControllerTest extends TestCase
{
    ...
    public function test_a_real_totp_code_confirms_and_enables_mfa_with_recovery_codes(): void
    {
        ...
        $generate = $this->actingAs($user)->postJson('/api/security/two-factor/generate');
        $generate->assertOk();
        ...
        $confirm = $this->actingAs($user)->postJson('/api/security/two-factor/confirm', ['code' => $code]);
        $confirm->assertCreated();
        $this->assertCount(8, $confirm->json('recovery_codes'));

        $this->assertTrue($user->fresh()->hasMfaEnabled());
        $this->actingAs($user)->getJson('/api/security/two-factor')->assertOk()->assertJsonPath('enabled', true);
    }

    public function test_a_wrong_code_does_not_confirm_mfa(): void
    {
        $user = User::factory()->create();
        $this->actingAs($user)->postJson('/api/security/two-factor/generate');

        $this->actingAs($user)
            ->postJson('/api/security/two-factor/confirm', ['code' => '000000'])
            ->assertStatus(422);
    ...
    public function test_login_requires_a_real_mfa_code_once_enabled(): void
    {
        ...
        $this->postJson('/api/login', ['email' => 'mfa@example.com', 'password' => 'password123'])->assertOk();

        $generate = $this->postJson('/api/security/two-factor/generate');
        $secret = $generate->json('secret');
        $code = (new Google2FA)->getCurrentOtp($secret);
        $this->postJson('/api/security/two-factor/confirm', ['code' => $code])->assertCreated();
        ...
        $login = $this->postJson('/api/login', ['email' => 'mfa@example.com', 'password' => 'password123']);
        $login->assertOk()->assertJsonPath('requires_mfa', true);
        $this->getJson('/api/user')->assertStatus(401);

        // Wrong MFA code is rejected.
        $this->postJson('/api/login/mfa', ['code' => '000000'])->assertStatus(422);

        // Real MFA code completes the login.
        $freshCode = (new Google2FA)->getCurrentOtp($secret);
        $this->postJson('/api/login/mfa', ['code' => $freshCode])->assertOk()->assertJsonPath('email', 'mfa@example.com');
        $this->getJson('/api/user')->assertOk()->assertJsonPath('email', 'mfa@example.com');
    }
    ...
    public function test_a_recovery_code_can_be_used_once_and_then_is_rejected(): void
    {
        ...
        $this->postJson('/api/login', ['email' => 'recovery@example.com', 'password' => 'password123'])->assertOk();

        $generate = $this->postJson('/api/security/two-factor/generate');
        $secret = $generate->json('secret');
        $code = (new Google2FA)->getCurrentOtp($secret);
        $confirm = $this->postJson('/api/security/two-factor/confirm', ['code' => $code]);
        $recoveryCode = $confirm->json('recovery_codes')[0];

        $this->postJson('/api/logout');
        Auth::forgetGuards();

        $this->postJson('/api/login', ['email' => 'recovery@example.com', 'password' => 'password123']);
        $this->postJson('/api/login/mfa', ['code' => $recoveryCode])->assertOk();

        // Log out and try to reuse the SAME recovery code — must fail now.
        $this->postJson('/api/logout');
        Auth::forgetGuards();
        $this->postJson('/api/login', ['email' => 'recovery@example.com', 'password' => 'password123']);
        $this->postJson('/api/login/mfa', ['code' => $recoveryCode])->assertStatus(422);
    }
    ...
    public function test_disabling_mfa_removes_the_login_challenge(): void
    {
        ...
        $secret = $generate->json('secret');
        $code = (new Google2FA)->getCurrentOtp($secret);
        $this->actingAs($user)->postJson('/api/security/two-factor/confirm', ['code' => $code]);

        $this->actingAs($user)->deleteJson('/api/security/two-factor')->assertNoContent();
        $this->assertFalse($user->fresh()->hasMfaEnabled());

        $login = $this->postJson('/api/login', ['email' => 'disable@example.com', 'password' => 'password123']);
