...
class AuditLogTest extends TestCase
{
    ...
    public function test_changing_a_team_members_role_writes_a_real_audit_row(): void
    {
        ...
        $this->actingAs($owner)
            ->patchJson("/api/teams/{$team->id}/members/{$membership->id}", ['role' => 'admin'])
            ->assertOk();
    ...
    {
        $owner = User::factory()->create();
        $project = $this->actingAs($owner)->postJson('/api/projects', ['title' => 'Delete Me'])->json();
    ...
    {
        $owner = User::factory()->create();
        $project = $this->actingAs($owner)->postJson('/api/projects', ['title' => 'Feature Audit'])->json();
        ...
        $store = $this->actingAs($owner)->postJson("/api/projects/{$project['id']}/features", ['prompt' => 'add an entrypoint']);
        ...
        $job = $this->actingAs($owner)->getJson("/api/ai-jobs/{$store->json('job_id')}");
        $featureRequestId = $job->json('result.feature_request_id');

        $this->actingAs($owner)->postJson("/api/projects/{$project['id']}/features/{$featureRequestId}/plan/approve", [
            'approved_paths' => ['app.js'],
        ])->assertOk();

        $this->actingAs($owner)->postJson("/api/projects/{$project['id']}/features/{$featureRequestId}/generate", []);

        $this->actingAs($owner)->postJson("/api/projects/{$project['id']}/features/{$featureRequestId}/diff/approve", [
            'approved_paths' => ['app.js'],
        ])->assertOk();

        $this->actingAs($owner)->postJson("/api/projects/{$project['id']}/features/{$featureRequestId}/apply", [
